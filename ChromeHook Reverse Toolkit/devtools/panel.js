...
const decodeEscapesButton = document.getElementById("decodeEscapes");
const scanAntiDebugButton = document.getElementById("scanAntiDebug");
...
function formatPageLabel(pageUrl) {
  try {
    return pageUrl ? new URL(pageUrl).hostname : "当前标签页";
  } catch (_error) {
    return pageUrl || "当前标签页";
  }
}
...
function decodeEscapedText(source) {
  return source
...
function scanAntiDebugPatterns(source) {
  const patterns = [
    { name: "debugger statement", regex: /\bdebugger\b/g },
    { name: "Function constructor", regex: /\bFunction\s*\(/g },
    { name: "eval call", regex: /\beval\s*\(/g },
    { name: "setInterval loop", regex: /\bsetInterval\s*\(/g },
    { name: "setTimeout loop", regex: /\bsetTimeout\s*\(/g },
    { name: "console.clear", regex: /console\.clear\s*\(/g },
    { name: "devtools keyword", regex: /devtools|__defineGetter__|outerWidth|outerHeight|performance\.now|Date\.now/gi }
  ];

  const findings = patterns
    .map((pattern) => {
      const matches = [...source.matchAll(pattern.regex)];
      return matches.length ? `${pattern.name}: ${matches.length} 处` : null;
    })
    .filter(Boolean);
...
function renderStatus() {
  ...
  statusBadge.className = `status-badge ${installed ? "live" : "pending"}`;
...
decodeEscapesButton.addEventListener("click", () => {
  const source = analysisInput.value || "";
  if (!source.trim()) {
    setAnalysisOutput("请先输入需要解码的内容。");
    return;
  }

  setAnalysisOutput(decodeEscapedText(source));
});

scanAntiDebugButton.addEventListener("click", () => {
  const source = analysisInput.value || "";
  if (!source.trim()) {
    setAnalysisOutput("请先输入需要扫描的代码片段。");
    return;
  }

  setAnalysisOutput(scanAntiDebugPatterns(source));
});

clearAnalysisButton.addEventListener("click", () => {
  analysisInput.value = "";
  setAnalysisOutput("等待输入分析内容。");
});
...
document.querySelectorAll("[data-template]").forEach((button) => {
  button.addEventListener("click", () => {
    customCode.value = templates[button.getAttribute("data-template")] || templates.atob;
    savePreferences().catch(() => {});
  });
});
